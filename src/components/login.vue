<template>
  <div class="dialog-wrapper">
    <form id="loginForm">
      <span class="delete-btn"><router-link to="/">x</router-link></span>
      ...
      <div class="item">
        ...
        />
        <input v-else type="email" maxlength="40" v-model="emailValue" autocomplete="on" placeholder="请输入邮箱" />
      </div>
      <!-- <div class="item"> 
  ...
  methods:{
      ...
      handleLogin(){
        ...
        let fromData = new FormData(form);
         this.$http.post(this.INTERFACE.login,fromData).then(res=>{
